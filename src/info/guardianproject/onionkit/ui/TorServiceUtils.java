...
public class TorServiceUtils {
	...
	public static boolean isRootPossible() {
		...
		try {
			...
			if (fileSU.exists()) {
				String[] cmd = { "su" };
				int exitCode = TorServiceUtils.doShellCommand(cmd, log, false,
						true);
				if (exitCode != 0)
					return false;
				else
			...
			// Check for 'su' binary
			String[] cmd = { "which su" };
			int exitCode = TorServiceUtils
					.doShellCommand(cmd, log, false, true);
	...
	public static int findProcessId(String command) {
		...
		try {
			...
			try {
				...
				Log.e(TAG,
						"Unable to get proc id for command: "
	...
	public static int findProcessIdWithPidOf(String command) throws Exception {
		...
		while ((line = reader.readLine()) != null) {
			...
			try {
				...
				Log.e("TorServiceUtils",
						"unable to parse process pid: " + line, e);
			}
		}
