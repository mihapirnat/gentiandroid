...
public class GentianConfig extends Node {
	public final List<GentianAccount> accounts=Collections.synchronizedList(new ArrayList<GentianAccount>());
	...
	public GentianAccount getAccount(String host, int port, String user) {
		for (GentianAccount acc : accounts) {
			if (acc.getServer().equals(host) && acc.getPort()==port && acc.getUser().equals(user)) {
				return acc;
			}
		}
		return null;
		...
		for (GentianAccount acc : accounts) {
			if (acc.getServer().equals(host) && acc.getPort()==port) {
				...
				for (GentianBuddy buddy : acc.buddies) {
					System.out.println("Comparing "+buddy.getUser()+" and "+user);
