...
//		 buddy.unset(TMPKEYDECRYPT); // TO DEBUG RSA ENCRYPTION
		...
		try {
			...
			if (buddy != null) {
				if (buddy.get(TMPKEYDECRYPT) != null
					...
					byte[] keybytes = Base64.decode(buddy.get(TMPKEYDECRYPT));
					byte[] msg = null;
					try {
						if (buddy.get(TMPECDHPRIV1)!=null && buddy.get(TMPECDHPRIV2)!=null && buddy.get(TMPECDHPUB1)!=null && buddy.get(TMPECDHPUB2)!=null) {
							KeyAgreement aKeyAgree = KeyAgreement.getInstance("ECDH", "SC");

							aKeyAgree.init(decodePrivate(Base64.decode(buddy.get(TMPECDHPRIV1))));
							aKeyAgree.doPhase(decodePublic(Base64.decode(buddy.get(TMPECDHPUB1))), true);
							
							byte[] key1 = deriveKeyFromSecret(aKeyAgree.generateSecret());
							KeyAgreement bKeyAgree = KeyAgreement.getInstance("ECDH", "SC");

							bKeyAgree.init(decodePrivate(Base64.decode(buddy.get(TMPECDHPRIV2))));
							bKeyAgree.doPhase(decodePublic(Base64.decode(buddy.get(TMPECDHPUB2))), true);
							
							byte[] key2 = deriveKeyFromSecret(bKeyAgree.generateSecret());
							keybytes=join(keybytes, key1,true,false);
							keybytes=join(keybytes, key2,true,false);
							if (buddy.get(TMPKEYENCRYPT)!=null) {
								buddy.set(TMPKEYDECRYPT, Base64.encodeToString(keybytes, false));
								byte[] keybytes2=Base64.decode(buddy.get(TMPKEYENCRYPT));
								keybytes2=join(keybytes2, key1,true,false);
								keybytes2=join(keybytes2, key2,true,false);
								buddy.set(TMPKEYENCRYPT, Base64.encodeToString(keybytes2, false));
								buddy.unset(TMPECDHPRIV1);
								buddy.unset(TMPECDHPRIV2);
								buddy.unset(TMPECDHPUB1);
								buddy.unset(TMPECDHPUB2);
								wipe(keybytes2);
								if (debug)System.out.println("added ECDH encrypt keys");
							} else {
								if (debug)System.out.println("using but not added ECDH encrypt keys");
							}
							wipe(key1);
							wipe(key2);
						...
						for (int i = keys.length - 1; i >= 0; i--) {
							if (debug) {
								System.out.println("Using key: "+Base64.encodeToString(keys[i], false));
							}
						...
						if (Util.equal(digest, digesttest)) {
							...
							buddy.set(TIMESTAMPDECRYPT, Long
									.toString(timestamp));
							...
							return Messages.staticmain.getText(R.string.cant_decode_received_sms).toString();
						}
			...
			try {
				...
				buddy.set(TMPECDHPUB1, Base64.encodeToString(ecpublicbytes[0], false));
				buddy.set(TMPECDHPUB2, Base64.encodeToString(ecpublicbytes[1], false));
				if (debug) {
				...
				if (extrabytes != null) {
					...
					for (int i = keys.length - 1; i >= 0; i--) {
						if (debug) {
							System.out.println("Extra using key: "+Base64.encodeToString(keys[i], false));
						}
				...
				byte[] testbytes = null;
				if (buddy != null) {
					testbytes = join(ecpublicbytes[0],join(ecpublicbytes[1],join(hmac, join(keybytes, msg, false, false),
				...
				if (buddy == null
					...
					if (buddy != null) {
						...
						if (lasttimestring != null) {
							try {
								lasttimestamp = Long.parseLong(lasttimestring);
								if (debug)
					...
					String s = new String(msg, "utf-8")
							...
							+ String.format("%1$tY/%1$tm/%1$td %1$tH:%1$tM",
									calendar) + "]";
					if (buddy != null) {
						buddy.set(TMPKEYDECRYPT, Base64.encodeToString(
								keybytes, false));
						buddy.set(TMPHMACDECRYPT, Base64.encodeToString(hmac,
								false));
						buddy.set(TIMESTAMPDECRYPT, Long.toString(timestamp));
						buddy.set(COUNTDECRYPT, Integer.toString(1));
						if (code.equals(CODE_NEW_KEY)) {

							buddy.unset(TMPKEYENCRYPT);
						}
						if (buddy.get(TMPECDHPRIV1)==null || buddy.get(TMPECDHPRIV2)==null) {
							buddy.unset(TMPKEYENCRYPT);
						}
					...
					return Messages.staticmain
							.getText(R.string.wrong_signature).toString();
				}
	...
	//	 buddy.unset(TMPKEYENCRYPT); // TO DEBUG RSA ENCRYPTION
			...
			try {
				...
				if (buddy.get(TMPECDHPRIV1)!=null && buddy.get(TMPECDHPRIV2)!=null && buddy.get(TMPECDHPUB1)!=null && buddy.get(TMPECDHPUB2)!=null) {
					...
					if (buddy.get(TMPKEYDECRYPT)!=null) {
						...
						byte[] keybytes2=Base64.decode(buddy.get(TMPKEYDECRYPT));
						keybytes2=join(keybytes2, key1,true,false);
						keybytes2=join(keybytes2, key2,true,false);
						buddy.set(TMPKEYDECRYPT, Base64.encodeToString(keybytes2, false));
						buddy.unset(TMPECDHPRIV1);
						buddy.unset(TMPECDHPRIV2);
						buddy.unset(TMPECDHPUB1);
						buddy.unset(TMPECDHPUB2);
		...
		try {
			...
			buddy.set(TMPECDHPRIV2, Base64.encodeToString(bKeyPair.getPrivate().getEncoded(), false));
			 ...
			 */
			ByteArrayOutputStream bout = new ByteArrayOutputStream();
			byte[] signbytes = join(aKeyPair.getPublic().getEncoded(),join(bKeyPair.getPublic().getEncoded(),join(hmac, join(keybytes, b, false, false),
					false, true),false,true),false,true);
			bout.write(buddy.getAccount().signOutgoing(signbytes));
			wipe(signbytes);
			// bout.write(digest);
			bout.write(b);
			b = bout.toByteArray();
			...
			bout.write(keybytes);
			bout.write(hmac);
			buddy.set(TMPECDHPRIV1, Base64.encodeToString(aKeyPair.getPrivate().getEncoded(), false));
			if (debug) {
				System.out.println("Sending public EC 1:"+Base64.encodeToString(aKeyPair.getPublic().getEncoded(), false));
			}
			bout.write(aKeyPair.getPublic().getEncoded());
			buddy.set(TMPECDHPRIV2, Base64.encodeToString(bKeyPair.getPrivate().getEncoded(), false));
			if (debug) {
				System.out.println("Sending public EC 2:"+Base64.encodeToString(bKeyPair.getPublic().getEncoded(), false));
			}
	...
	private static void testECDH() {
		try {
			...
			System.out.println("a public encoded length:"+aKeyPair.getPublic().getEncoded().length);
			System.out.println("a private encoded length:"+aKeyPair.getPrivate().getEncoded().length);
			//
			// b side
			//
			KeyPair bKeyPair = g.generateKeyPair();
			...
			System.out.println("b public encoded length:"+bKeyPair.getPublic().getEncoded().length);
			System.out.println("b private encoded length:"+bKeyPair.getPrivate().getEncoded().length);
			//
			// agreement
			//
			...
			if (!k1.equals(k2)) {
				...
				System.out.println("OK ;) ECDH 2-way test passed: "
						+ aSecret.length+" derived key: "+Base64.encodeToString(deriveKeyFromSecret(aSecret), false));
			}
			try {
	...
	public static boolean forwardSecrecy(GentianBuddy buddy) {
		if (buddy.get(TMPKEYENCRYPT)!=null && Base64.decode(buddy.get(TMPKEYENCRYPT)).length>KEYSLEN) return true;
		return buddy.get(TMPECDHPRIV1)!=null && buddy.get(TMPECDHPRIV2)!=null && buddy.get(TMPECDHPUB1)!=null && buddy.get(TMPECDHPUB2)!=null;		
	}
	static byte[] deriveKeyFromSecret(byte[] secret) throws NoSuchAlgorithmException {
	...
	private static void testECpublicEncoding(PublicKey publicKey) throws NoSuchAlgorithmException, NoSuchProviderException, InvalidKeySpecException {
        byte[]              pubEnc = publicKey.getEncoded();
        ECPublicKey pubKey=decodePublic(pubEnc);

        if (!pubKey.getW().equals(((ECPublicKey)publicKey).getW()))
        {
            System.out.println(" expected " + pubKey.getW().getAffineX() + " got " + ((ECPublicKey)publicKey).getW().getAffineX());
            System.out.println(" expected " + pubKey.getW().getAffineY() + " got " + ((ECPublicKey)publicKey).getW().getAffineY());
            System.out.println("ECDH public key encoding (W test) failed");
        } else {
        	System.out.println("ECDH public key encoding (W test) OK!");
        }

        if (!pubKey.getParams().getGenerator().equals(((ECPublicKey)publicKey).getParams().getGenerator()))
        {
	...
	private static void testECprivateEncoding(PrivateKey privateKey) throws NoSuchAlgorithmException, NoSuchProviderException, InvalidKeySpecException {
        ...
        if (!privKey.getParams().getGenerator().equals(((ECPrivateKey)privateKey).getParams().getGenerator()))
        {
